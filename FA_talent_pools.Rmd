...
The data used in this analysis were the importance measures for each occupation. Furthermore, only observations that were not *recommended for suppression* were used (refer to page 1 in *Data Dictionary O\*NET 22.0 Database* manual). Observations with missing data were removed from the dataset. The resulting dataset contained `r nrow(ksa)` occupations and `r ncol(ksa)` variables. 
